    ...
    <>
      ...
      <div
        className={`w-full flex ${
          noGutterRoutes.includes(pathname) ? "bg-bg" : "bg-bg_shade"
        }`}
      >
        {/* Sidebar */}
        ...
        {/* Main content area */}
        <div
          className={`flex-grow sm+:px-6 px-3 min-h-screen overflow-hidden ${
            isSidebarCollapsed ? "xl:ml-[4.5rem]" : "xl:ml-[16rem]"
          } relative transition-all duration-300`}
        >
          <div
            className={`fixed top-0.5 right-0 ${
              isSidebarCollapsed ? "xl:left-[4.5rem]" : "xl:left-[16rem]"
            } left-0 bg-white z-[999] flex items-center transition-all duration-300`}
          >
            <MobileSidebar />
            <DashboardHeader />
          </div>
          <div className="mt-[70px] sm+:mt-24 mb-8 w-full">{children}</div>
        </div>
      </div>
    </>
  );
