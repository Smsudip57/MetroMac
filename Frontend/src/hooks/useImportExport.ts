...
import { useImportDataMutation, useLazyExportDataQuery } from "@/redux/api/files/importExportApi";
...
export function useExportData() {
    ...
    const exportData = async (module: string, format: "csv" | "xlsx" = "xlsx", filters?: Record<string, any>) => {
        try {
            setIsExporting(true);
            ...
            link.href = url;
            link.setAttribute("download", `${module}-export-${Date.now()}.${format === "csv" ? "csv" : "xlsx"}`);
...
export const validateImportFile = (file: File): { isValid: boolean; error?: string } => {
    ...
    if (file.size > maxSize) {
        return { isValid: false, error: `File size exceeds 10 MB limit (${(file.size / 1024 / 1024).toFixed(2)} MB)` };
    }
...
export const createExportParams = (params: ExportParams): Record<string, any> => {
